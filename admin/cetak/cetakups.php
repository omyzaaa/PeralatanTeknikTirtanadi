...
judul('IDENTIFIKASI KERUSAKAN UPS','U P S ( Uninterup Power Supply )', $pdf);
...
$pdf->Rect(10, 75, 190, 160);
$pdf->Line(10, 81, 200, 81); // X1, Y1, X2, Y2
$pdf->Line(10, 125, 200, 125); // X1, Y1, X2, Y2
$pdf->Line(10, 131, 200, 131); // X1, Y1, X2, Y2
$pdf->Line(10, 160, 200, 160); // X1, Y1, X2, Y2
$pdf->Line(10, 190, 200, 190); // X1, Y1, X2, Y2
$pdf->Line(60, 190, 60, 235);
$pdf->Line(150, 190, 150, 235);
...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    $query_pemeriksaan = "SELECT pemeriksaan FROM pem_ups";
...
$query_kerusakan = "SELECT kerusakan FROM ker_ups";
...
if ($result_kerusakan && mysqli_num_rows($result_kerusakan) > 0) {
    ...
    $pdf->SetXY(0, 121);; // Geser ke bawah sejauh 10 unit dari posisi terakhir
...
if ($result_rekomendasi && mysqli_num_rows($result_rekomendasi) > 0) {
    ...
    $pdf->SetFont('Arial', '', 12); // Atur font
    $pdf->SetXY(72, 192); // Geser ke bawah sejauh 10 unit dari posisi terakhir
